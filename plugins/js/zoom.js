+function () {
  ...
  Zooming.prototype = {
    ...
    _zoom: function (target) {
      ...
      console.log(this._imgtarget)
      console.log(target)
     
       
        clearTimeout(this._imgtarget._timer);
      



      target.parentNode.parentNode.setAttribute('class', '')
      this._target = new Zoomable(target)
      this._target.zoomIn();
      this._imgtarget = target;
      console.log('zoom')
      console.log(target)
    ...
    _close: function () {
      var self = this;
      if (!this._target) return
      console.log(this._target._image)

      console.log('close')

      this._imgtarget._timer = setTimeout(function () {
        console.log('timer')
        self._imgtarget.parentNode.parentNode.setAttribute('class', 'adListItemWrap')

      }, 1000)
      // self._imgtarget.setAttribute('data-action', 'zoom')
